"""
Breast Cancer — PINN + 最小二乘量子预设
PDE 约束 + 量子 CIM LS-QUBO 预设解作为监督锚点
"""
import numpy as np
...
print(f'PINN+LS vs FEM: L2={l2_pinn:.4e}, 内部RMSE={rmse_i:.4e}')

np.save(f'{OUTPUT_DIR}/u_pinn_ls.npy', u_pinn)
res = {'method':'pinn_ls_preset','epochs':N_epoch,'time_sec':elapsed,
       'L2_vs_FEM':float(l2_pinn),'RMSE_internal':float(rmse_i),'final_loss':hist['loss'][-1]}
with open(f'{OUTPUT_DIR}/result_pinn_ls.json','w') as f: json.dump(res, f, indent=2)
print('结果已保存')
