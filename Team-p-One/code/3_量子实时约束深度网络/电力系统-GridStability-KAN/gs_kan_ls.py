"""
Grid Stability — KAN + PDE约束 + 最小二乘量子预设
KAN with PDE residual + least-squares QUBO preset supervision
"""
import numpy as np
...
preset_nodes = np.load(f'{OUTPUT_DIR}/preset_nodes_ls.npy')
preset_values = np.load(f'{OUTPUT_DIR}/preset_values_ls.npy')
...
print(f'KAN+LS vs FEM: L2={l2:.4e}, 内部RMSE={rmse_i:.4e}')

np.save(f'{OUTPUT_DIR}/u_kan_ls.npy', u_kan)
res = {'method':'kan_ls_preset','epochs':N_epoch,'time_sec':elapsed,'L2_vs_FEM':float(l2),'RMSE_internal':float(rmse_i)}
with open(f'{OUTPUT_DIR}/result_kan_ls.json','w') as f: json.dump(res, f, indent=2)
print('结果已保存')
